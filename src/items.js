...
export const ItemType = Object.freeze({
  Multi: 'M', Long: 'L', Short: 'S', Up: 'U', Down: 'D', Break: 'B'
});
...
export function applyItem(game, itemType) {
  const { paddle, balls } = game;

  switch (itemType) {
    case ItemType.Multi: {
      // 既存の1球から角度違いに2球生成
      const base = balls[0] || null;
      if (base) {
        game.balls.push(base.clone(-0.2));
        game.balls.push(base.clone(+0.2));
        game.updateBallCountUI();
      }
      break;
    }
    case ItemType.Long: {
    ...
    case ItemType.Up: {
      game.speedMultiplier = Math.min(game.speedMultiplier + 0.12, CONFIG.speedRamp.maxMultiplier);
      break;
    }
    case ItemType.Down: {
      game.speedMultiplier = Math.max(0.6, game.speedMultiplier - 0.12);
      break;
    }
    case ItemType.Break: {
